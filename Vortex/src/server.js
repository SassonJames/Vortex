...
let gems = []; // Array to hold the gems that players will collect
...
const addVelocity = () => {
    if(drawstack.length != 0){
         // Vortex Velocity
        for(var i = 0; i < drawstack.length; i++){
            let circle = drawstack[i];
            let distY = Math.abs(250-circle.y);
            let distX = Math.abs(250-circle.x);
            let ratY = 0;
            let ratX = 0;
...
const generateGems = () => {
    var prevGems = gems.length;
    while(gems.length < numPlayers*2){
        var gem = {
            x: Math.floor(Math.random()*4000 + 500),    // The gem x and y look strange. They need to scale for the picture and draws to work. Look at the Draw function in index.html for more information
            y: Math.floor(Math.random()*4000 + 500)     // The gem x and y look strange. They need to scale for the picture and draws to work. Look at the Draw function in index.html for more information
        }
        gems.push(gem);
    }
